...
class SfMLearner(object):
    ...
    def build_train_graph(self):
        ...
        with tf.name_scope("compute_loss"):
            ...
            for s in range(opt.num_scales):
                ...
                for i in range(opt.num_source):
                    if self.icp_weight > 0:
                        multiscale_clouds_i = [
                            project.get_cloud(d,
                                              intrinsic_mat_inv[:, 0, :, :], #这个变量要可能不同，在reader里查看同dataloader对比
                                              name='cloud%d_%d' % (s, i))
                            for (s, d) in enumerate(pred_depth)
                        ]
                ...
                #对所有图像做点云
                    ...
                    if opt.icp_weight > 0:
                        ...
                        for i in range(self.seq_length):
                            ...
                            tgt_pcl=self.cloud[i]
                            src_pcl=self.cloud[i+1]
            ...
            total_loss = pixel_loss + smooth_loss + exp_loss + opt.icp_weight*icp_loss
