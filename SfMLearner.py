...
class SfMLearner(object):
    ...
    def build_train_graph(self):
        ...
        with tf.name_scope("depth_prediction"):
            ...
            def get_src_depth(src_image_stack)
                tf.reset_default_graph()
                with tf.name_scope("src_depth_prediction"):
                    src2_disp, src_net_endpoints = src_disp_net(src_image_stack[:, :, :, 4:6],
                                                                is_training=False)
                    src_pred_depth = [1 / d1 for d1 in src2_disp]
                return src_pred_depth

            src_pred_depth=get_src_depth(src_image_stack)
            for s in range(len(src_pred_depth)):
                tgt_cloud = project.get_cloud(pred_depth,
                                              intrinsic_mat_inv[:, s, :, :])
                src_cloud = project.get_cloud(src_pred_depth,
                                              intrinsic_mat_inv[:, s, :, :])
        ...
        with tf.name_scope("compute_loss"):
            ...
            if self.icp_weight > 0:
                ...
                centroids1 = tf.reduce_mean(
                    t2s_pcl, axis=2, keep_dims=True)
                centroids2 = tf.reduce_mean(
                    src_pcl, axis=2, keep_dims=True)
            ...
            for s in range(opt.num_scales):
                ...
                for i in range(opt.num_source):
                    ...
                    curr_proj_image, src_cam_coords = projective_inverse_warp(
    ...
    def preprocess_image(self, image):
        ...
        image = tf.image.convert_image_dtype(image, dtype=tf.float32)  # 图像转化为th.float32类型
        return image * 2. - 1.
